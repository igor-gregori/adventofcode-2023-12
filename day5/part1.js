...
function findDestinationValue(sourceValue, map) {
    for (const range of map) {
        ...
        if (isBetween(sourceValue, destinationRangeStart, destinationRangeStart + (rangeLength - 1))) {
...
for (const location of locations) if (lowestLocation > location) lowestLocation = location
