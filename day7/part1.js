...
function getHandType(handCards) {
    ...
    let qttOfMostRepeatedCard = cards[0][1]
    for (const [_, count] of cards) {
        if (count > qttOfMostRepeatedCard) qttOfMostRepeatedCard = count
    }

    if (qttOfMostRepeatedCard === 5) return 'fiveOfAKind'
    if (qttOfMostRepeatedCard === 4) return 'fourOfAKind'
    if (qttOfMostRepeatedCard === 3) {
        for (const [_, count] of cards) if (count === 2) return 'fullHouse'
        return 'threeOfAKind'
    }
...
function compareStrength(handX, handY) {
    ...
    const cardsInOrderOfStrength = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2']
