...
function readLines(filePath) {
  ...
  for (const line of lines) {
    ...
    for (const subGameResult of subGameResults) {
      ...
      for (const unitGameResult of unitGameResults) {
        if (unitGameResult.includes("red"))
          red = Number(unitGameResult.replace("red", ""));
        if (unitGameResult.includes("green"))
          green = Number(unitGameResult.replace("green", ""));
        if (unitGameResult.includes("blue"))
          blue = Number(unitGameResult.replace("blue", ""));
      }
  ...
  for (const gameData of totalGameData) {
    ...
    const multiplicationOfMinimum =
      minimunRedCubes * minimunGreenCubes * minimunBlueCubes;
    minimumGames.push([
      numberOfGame,
      [minimunRedCubes, minimunGreenCubes, minimunBlueCubes],
      multiplicationOfMinimum,
    ]);
  }

  let sumOfMultiplicationOfMinimum = 0;
  for (const minimumGame of minimumGames)
    sumOfMultiplicationOfMinimum += minimumGame[2];
